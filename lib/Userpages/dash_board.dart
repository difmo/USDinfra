...
class _HomeDashBoard extends State<HomeDashBoard> {
  ...
  //   });
    ...
    Future<void> _getCurrentUserFavorites() async {
      ...
      if (user != null) {
        userId = user.uid;
        final doc = await FirebaseFirestore.instance.collection("AppUsers").doc(
            userId).get();
        if (doc.exists) {
          setState(() {
            favoriteProperties =
    ...
    Future<void> _toggleFavorite(String propertyId) async {
      ...
      final docRef = FirebaseFirestore.instance.collection("AppUsers").doc(
          userId);
                ...
                child: StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    snapshot.data!.docs.map((doc) async {
                      ...
                      final DateTime createdAtDate = createdAtTimestamp
                          ?.toDate() ?? DateTime.now();
                      final int daysAgo = DateTime
                          .now()
                          .difference(createdAtDate)
                          .inDays;
                      final String createdAtString = daysAgo > 0
                          ? '$daysAgo days'
                          : 'Today';
